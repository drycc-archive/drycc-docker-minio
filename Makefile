...
SHELL_SCRIPTS = $(shell find ${MINIO_VERSION}/debian/ -name '*.sh') $(wildcard *.sh)
...
docker-build: check-docker
	docker build ${DOCKER_BUILD_FLAGS} -t ${IMAGE} ${MINIO_VERSION}/debian
	docker tag ${IMAGE} ${MUTABLE_IMAGE}

docker-buildx: check-docker
	docker buildx build --platform ${PLATFORM} -t ${IMAGE} ${MINIO_VERSION}/debian --push
